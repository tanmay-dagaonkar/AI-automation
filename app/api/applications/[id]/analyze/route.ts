...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const row = await collection.findOneAndUpdate({ _id: new ObjectId(id), status: { $in: ["pending_approval", "failed"] } }, { $set: { status: "processing", updatedAt: new Date() }, $unset: { error: "" } }, { returnDocument: "after" });
  ...
  catch (error) { const message = error instanceof Error ? error.message : "Analysis failed."; await collection.updateOne({ _id: row._id }, { $set: { status: "failed", error: message.slice(0, 500), updatedAt: new Date() } }); return Response.json({ error: message }, { status: 502 }); }
}
